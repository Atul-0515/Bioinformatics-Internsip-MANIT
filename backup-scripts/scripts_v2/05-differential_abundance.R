...
# Summary reporting functions to add to your script
...
log_significant_taxa <- function(report_file, all_results) {
  ...
  for (level in names(all_results)) {
    ...
    # >>> START: ADD THIS NEW CODE BLOCK FOR LEFSE <<<
    if (!is.null(level_results$lefse) && nrow(level_results$lefse) > 0) {
      res <- level_results$lefse
      has_significant <- TRUE
      res <- res[order(res$p_value_bh), ] # Order by adjusted p-value
      top_taxa <- head(res, 5)
    ...
    # >>> END: ADD THIS NEW CODE BLOCK FOR LEFSE <<<
    if (has_significant) {
...
main <- function() {
  ...
  report_file <- initialize_summary_report() # ADD THIS LINE
  ...
  log_data_summary(report_file, abundance_data, metadata) # ADD THIS LINE
  ...
  # Log method summary  # ADD THIS LINE
  log_method_summary(report_file, all_results) # ADD THIS LINE

  # Log significant taxa details  # ADD THIS LINE
  log_significant_taxa(report_file, all_results) # ADD THIS LINE

  # Finalize report  # ADD THIS LINE
  finalize_report(report_file, all_results) # ADD THIS LINE
