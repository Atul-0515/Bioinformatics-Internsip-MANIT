...
def check_database():
    ...
    print("Please download a database and place the .tar.gz file in the 'databases' directory")
    return None
...
def get_available_read_lengths(db_path):
    ...
    for file in kmer_files:
        ...
        if "database" in filename and "mers.kmer_distrib" in filename:
            length_str = filename.replace("database", "").replace("mers.kmer_distrib", "")
            try:
...
def process_individual_kraken2(r1_file, r2_file, srr_id, db_path, threads=8):
    ...
    if success and kraken_output_file.exists() and kraken_report_file.exists():
        ...
        with open(kraken_report_file) as f:
            ...
            print(f"Kraken2 report has {len(classified_lines)} classified taxa for {srr_id}")
        return True
...
def create_abundance_tables():
    ...
    for level_code, level_name in levels.items():
        ...
        if merged_data:
            ...
            relative_abundance.to_csv(output_dir / f"{level_name}_relative_abundance.csv")
            print(f"Saved {level_name} tables with {len(abundance_df)} taxa and {len(sample_srr_ids)} samples")
...
def generate_taxonomy_summary():
    ...
    for level in levels:
        abundance_file = f"results/taxonomy/merged_tables/{level}_relative_abundance.csv"
...
def write_processing_report(samples_to_process, successful_samples, sample_category_map):
    """Write a detailed processing report"""
    report_path = Path("results/taxonomy/reports/processing_summary.txt")

    with open(report_path, "w") as f:
        f.write("TAXONOMIC PROFILING PROCESSING REPORT\n")
        f.write("=" * 50 + "\n\n")

        f.write(f"Total samples found: {len(samples_to_process)}\n")
        f.write(f"Successfully processed: {successful_samples}\n")
        f.write(f"Failed samples: {len(samples_to_process) - successful_samples}\n\n")
        ...
        f.write("SAMPLES BY CATEGORY:\n")
        f.write("-" * 20 + "\n")
        for category, count in sorted(category_counts.items()):
            f.write(f"{category}: {count} samples\n")

        f.write(f"\nProcessing success rate: {(successful_samples / len(samples_to_process) * 100):.1f}%\n")


def write_taxonomy_summary_report():
    """Write a human-readable taxonomy summary report"""
    report_path = Path("results/taxonomy/reports/taxonomy_summary.txt")

    with open(report_path, "w") as f:
        f.write("TAXONOMIC PROFILING RESULTS SUMMARY\n")
        f.write("=" * 50 + "\n\n")
        ...
        for level in levels:
            ...
            if Path(abundance_file).exists():
                try:
                    ...
                    f.write("-" * 15 + "\n")
                    f.write(f"Total {level} detected: {len(df)}\n")
                    f.write(f"Samples analyzed: {len(df.columns)}\n")

                    # Top 5 most abundant taxa
                    mean_abundance = df.mean(axis=1).sort_values(ascending=False)
                    f.write(f"Top 5 most abundant {level}:\n")
                    for i, (taxon, abundance) in enumerate(mean_abundance.head(5).items(), 1):
                        f.write(f"  {i}. {taxon}: {abundance:.2f}% (average)\n")

                    f.write("\n")

                except Exception as e:
                    f.write(f"Error processing {level} level: {e}\n\n")

        f.write("Files generated:\n")
        f.write("- Raw abundance tables: results/taxonomy/merged_tables/*_abundance_counts.csv\n")
        f.write("- Relative abundance tables: results/taxonomy/merged_tables/*_relative_abundance.csv\n")
        f.write("- Sample categories: results/taxonomy/sample_categories.csv\n")
        f.write("- Processing logs: logs/\n")


def write_final_report():
    """Write a concise final summary report"""
    report_path = Path("results/taxonomy/reports/final_summary.txt")

    # Read the JSON summary for data
    json_path = Path("results/taxonomy/taxonomy_summary.json")
    ...
    with open(report_path, "w") as f:
        f.write("TAXONOMIC PROFILING - FINAL SUMMARY\n")
        f.write("=" * 50 + "\n\n")

        f.write("✓ Analysis completed successfully\n")
        f.write("✓ {summary_data['total_samples_processed']} samples processed\n\n")

        f.write("TAXONOMIC DIVERSITY DETECTED:\n")
        for level, count in summary_data["total_taxa_detected"].items():
            f.write(f"  {level.capitalize()}: {count} taxa\n")

        f.write("\nResults are ready for downstream analysis in R or other tools.\n")
        f.write("Sample metadata saved for statistical analysis.\n\n")

        f.write("Next steps:\n")
        f.write("- Use sample_categories.csv for grouping samples\n")
        f.write("- Analyze relative_abundance.csv files for diversity metrics\n")
        f.write("- Perform statistical comparisons between categories\n")
...
def main():
    ...
    for category_dir in category_dirs:
        ...
        for r1_file in r1_files:
            r2_file = r1_file.parent / r1_file.name.replace("_1_trimmed.fastq.gz", "_2_trimmed.fastq.gz")
    ...
    for sample_info in samples_to_process:
        ...
        if process_individual_kraken2(sample_info["r1_path"], sample_info["r2_path"], srr_id, db_path):
    ...
    # After the processing loop ends (around line where you print processing complete)

    print(f"\nProcessing complete: {successful_samples}/{len(samples_to_process)} samples successful")

    # Write processing report
    write_processing_report(samples_to_process, successful_samples, sample_category_map)
    ...
    if successful_samples > 0:
        ...
        write_taxonomy_summary_report()
        write_final_report()

        print("\nTaxonomic profiling complete! Results in results/taxonomy/")
        print("Summary reports saved in results/taxonomy/reports/")
    else:
