...
class MicrobiomeBiomarkerDiscovery:
    ...
    def load_taxonomic_data(self):
        ...
        metadata_file = os.path.join(
            self.results_dir, "taxonomy", "sample_categories.csv"
        )
        if not os.path.exists(metadata_file):
            raise FileNotFoundError(
                "Sample categories file not found. Run taxonomic profiling first."
            )
        ...
        for level in levels:
            abundance_file = os.path.join(
                taxonomy_dir, f"{level}_relative_abundance.csv"
            )
            if os.path.exists(abundance_file):
                ...
                print(
                    f"Loaded {level} data: {len(df[abundant_taxa])} taxa, {len(df.columns)} samples"
                )
    ...
    def load_differential_results(self):
        ...
        if not os.path.exists(diff_dir):
            print(
                "Warning: Differential abundance results not found. Proceeding without feature prioritization."
            )
            return
        ...
        for level in self.abundance_data.keys():
            ...
            # Load DESeq2 results
            deseq_files = [
                f for f in os.listdir(diff_dir) if f.startswith(f"{level}_deseq2_")
            ]
            for file in deseq_files:
                ...
                if not significant.empty:
                    self.differential_results[level][f"deseq2_{comparison}"] = (
                        significant
                    )

            # Load ALDEx2 results
            aldex_files = [
                f for f in os.listdir(diff_dir) if f.startswith(f"{level}_aldex2_")
            ]
            for file in aldex_files:
                comparison = file.replace(f"{level}_aldex2_", "").replace(".csv", "")
                df = pd.read_csv(os.path.join(diff_dir, file))
                significant = df[
                    (df["p_value_bh"] < 0.05) & (abs(df["effect_size"]) > 1)
                ]
                if not significant.empty:
                    self.differential_results[level][f"aldex2_{comparison}"] = (
                        significant
                    )

        print(
            f"Loaded differential results for {len(self.differential_results)} taxonomic levels"
        )
    ...
    def prepare_ml_data(self, level, target_column="Category"):
        ...
        y_metadata = self.metadata.set_index("SampleID").loc[
            common_samples, target_column
        ]
    ...
    def feature_selection(self, X, y, level, method="all", k=50):
        ...
        if method in ["all", "rfecv"]:
            ...
            selector = RFECV(
                estimator=rf, cv=cv, scoring="accuracy", min_features_to_select=5
            )
            selector.fit(X, y)
        ...
        # 4. Features from differential abundance
        if level in self.differential_results and method in ["all", "differential"]:
            ...
            for comparison, results in self.differential_results[level].items():
                if "taxon" in results.columns:
                    ...
                    if "log2FoldChange" in results.columns:
                        ...
                        sorted_results = results.reindex(
                    ...
                    valid_features = [
                        f for f in sorted_results["taxon"][:30] if f in X.columns
                    ]
            ...
            if diff_features:
                ...
                print(
                    f"    Added {len(diff_features)} differentially abundant features"
                )
        ...
        if len(selected_features) > 1:
            ...
            combined_features = [
                f[0] for f in sorted_features[: min(100, len(sorted_features))]
            ]
        else:
    ...
    def train_classifiers(self, X, y, class_names, level):
        ...
        for name, clf in classifiers.items():
            ...
            cv_precision = cross_val_score(
                clf, X_scaled, y, cv=cv, scoring="precision_macro"
            )
            ...
            print(
                f"    CV Accuracy: {results[name]['cv_accuracy_mean']:.3f} ± {results[name]['cv_accuracy_std']:.3f}"
            )
    ...
    def evaluate_models(self, results, class_names, level):
        ...
        cv_acc_mean = [results[m]["cv_accuracy_mean"] for m in models]
        cv_std = [results[m]["cv_accuracy_std"] for m in models]
        ...
        cm = confusion_matrix(
            results[best_model]["y_test"], results[best_model]["y_pred"]
        )
        ...
        if len(class_names) == 2:
            ...
            best_model = max(
                results.keys(), key=lambda x: results[x]["cv_accuracy_mean"]
            )
            result = results[best_model]

            # Binarize the output
            y_test_bin = label_binarize(
                result["y_test"], classes=range(len(class_names))
            )
        ...
        cv_data = [
            results[m]["cv_accuracy"] for m in models
        ]  # This is the array of CV scores
        ...
        report_file = os.path.join(
            self.ml_dir, "classification_reports", f"{level}_classification_report.txt"
        )
        with open(report_file, "w") as f:
            ...
            for name, result in results.items():
                ...
                f.write(
                    classification_report(
                        result["y_test"], result["y_pred"], target_names=class_names
                    )
                )
                f.write("\n" + "=" * 50 + "\n")
    ...
    def extract_biomarkers(self, results, feature_names, class_names, level):
        ...
        if not biomarker_df.empty:
            ...
            agg_biomarkers = (
                biomarker_df.groupby("feature")
                .agg({"importance": ["mean", "std", "count"], "level": "first"})
                .round(4)
            )
            ...
            agg_biomarkers = agg_biomarkers.sort_values(
                "mean_importance", ascending=False
            )

            # Save biomarker rankings
            output_file = os.path.join(
                self.ml_dir, "biomarker_panels", f"{level}_biomarker_ranking.csv"
            )
            ...
            plt.savefig(
                os.path.join(
                    self.ml_dir, "biomarker_panels", f"{level}_top_biomarkers.png"
                ),
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def dimensionality_reduction(self, X, y, class_names, level):
        ...
        for i, class_name in enumerate(class_names):
            mask = y == i
            axes[0].scatter(
                X_pca[mask, 0], X_pca[mask, 1], label=class_name, alpha=0.7, s=50
            )
        ...
        if len(X) > 50:  # Only run t-SNE if we have enough samples
            tsne = TSNE(
                n_components=2, random_state=42, perplexity=min(30, len(X) // 4)
            )
            ...
            for i, class_name in enumerate(class_names):
                mask = y == i
                axes[1].scatter(
                    X_tsne[mask, 0], X_tsne[mask, 1], label=class_name, alpha=0.7, s=50
                )
        ...
        plt.savefig(
            os.path.join(
                self.ml_dir, "dimensionality_reduction", f"{level}_dim_reduction.png"
            ),
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def save_models(self, results, level):
        ...
        for name, result in results.items():
            model_file = os.path.join(
                models_dir, f"{level}_{name.lower().replace(' ', '_')}_model.pkl"
            )
            scaler_file = os.path.join(
                models_dir, f"{level}_{name.lower().replace(' ', '_')}_scaler.pkl"
            )
    ...
    def run_analysis(self):
        ...
        for level in self.abundance_data.keys():
            ...
            try:
                ...
                # Extract biomarkers
                biomarkers = self.extract_biomarkers(
                    results, feature_names, class_names, level
                )
                if not biomarkers.empty:
                    all_biomarkers.append(biomarkers.head(10))  # Top 10 per level

                # Dimensionality reduction
                X_pca, variance_ratio = self.dimensionality_reduction(
                    X_selected, y, class_names, level
                )
                ...
                summary_results[level] = {
                    ...
                    "best_accuracy": results[best_model]["cv_accuracy_mean"],
    ...
    def create_summary_report(self, summary_results, all_biomarkers):
        ...
        with open(summary_file, "w") as f:
            ...
            f.write("-" * 20 + "\n")
            f.write(
                f"Sample Categories: {', '.join(self.metadata['Category'].unique())}\n"
            )
            ...
            if summary_results:
                ...
                f.write(
                    f"• Highest accuracy achieved: {summary_results[best_level]['best_accuracy']:.4f}\n"
                )
                f.write(
                    f"• Best model type: {summary_results[best_level]['best_model']}\n"
                )
        ...
        if all_biomarkers:
            ...
            if combined_biomarkers_list:
                combined_biomarkers = pd.concat(
                    combined_biomarkers_list, ignore_index=True
                )
                combined_biomarkers = combined_biomarkers.sort_values(
                    "mean_importance", ascending=False
                )

                combined_file = os.path.join(
                    self.ml_dir, "biomarker_panels", "combined_top_biomarkers.csv"
                )
                ...
                colors = [
                    level_colors.get(level, "gray") for level in top_combined["level"]
                ]

                y_pos = np.arange(len(top_combined))
                plt.barh(
                    y_pos, top_combined["mean_importance"], color=colors, alpha=0.7
                )
                ...
                legend_elements = [
                    plt.Rectangle(
                        (0, 0), 1, 1, facecolor=color, alpha=0.7, label=level.title()
                    )
                    for level, color in level_colors.items()
                    if level in top_combined["level"].values
                ]
                ...
                plt.savefig(
                    os.path.join(
                        self.ml_dir, "biomarker_panels", "combined_top_biomarkers.png"
                    ),
                    dpi=300,
                    bbox_inches="tight",
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ML Biomarker Discovery for Microbiome Data"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
