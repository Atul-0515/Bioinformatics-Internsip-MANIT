...
Uses Kraken2 and Bracken for taxonomy assignment and abundance estimation
    ...
    It correctly handles multiple trimmed files per sample directory
...
def write_processing_report(
    samples_to_process, successful_samples, sample_category_map
):
...
def write_taxonomy_summary_report():
    ...
    with open(report_path, "w") as f:
        ...
        for level in levels:
            ...
            if Path(abundance_file).exists():
                try:
                    ...
                    for i, (taxon, abundance) in enumerate(
                        mean_abundance.head(5).items(), 1
                    ):
        ...
        f.write(
            "- Raw abundance tables: results/taxonomy/merged_tables/*_abundance_counts.csv\n"
        )
        f.write(
            "- Relative abundance tables: results/taxonomy/merged_tables/*_relative_abundance.csv\n"
        )
