...
# Backup for genus_data
# cat("Attempting to load genus data...\n")
# genus_file <- "results/taxonomy/merged_tables/genus_relative_abundance.csv"
# cat("File exists:", file.exists(genus_file), "\n")

# if (file.exists(genus_file)) {
#   cat("File size:", file.size(genus_file), "bytes\n")
#   genus_data <- read.csv(genus_file, row.names = 1, check.names = FALSE)
#   cat("Genus data loaded. Dimensions:", dim(genus_data), "\n")
# } else {
#   cat("Genus file not found!\n")
#   genus_data <- NULL
# }
...
if (!is.null(genus_data)) {
  ...
  # Filter for top 15 most abundant genera (reduced for better visualization)
  genus_means <- rowMeans(genus_data)
  top_genera <- names(sort(genus_means, decreasing = TRUE)[1:15])
  genus_subset <- as.matrix(genus_data[top_genera, ])

  # Transform data for better visualization
  genus_log <- log10(genus_subset + 1e-6) # Better pseudocount
  ...
  colors <- colorRampPalette(c(
    ...
    "#D1E5F0", "#FFFFFF", "#FDBF6F", "#FF7F00",
    "#E31A1C", "#B10026"
  ))(50)

  png("results/final_visualizations/genus_heatmap.png",
    width = 14, height = 10, units = "in", res = 300
  )

  pheatmap::pheatmap(genus_log,
    ...
    fontsize_row = 10,
    fontsize = 9,
    main = "Taxonomic Composition: Top 15 Bacterial Genera",
    color = colors,
    border_color = "white",
    cellwidth = 6,
    cellheight = 18,
  ...
  dev.off()
  cat("Heatmap creation completed!\n")
}
...
create_beta_diversity_summary <- function(beta_results, alpha_results) {
  ...
  if (file.exists(metadata_file)) {
    ...
    # Create PCoA plots for genus level
